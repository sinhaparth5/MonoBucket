...
TEST_CASE("a session resolves to the administrator that opened it", "[session]") {
    SessionStore sessions;

    const std::string token = sessions.openAt("admin", kNow);
    CHECK(sessions.resolveAt(token, kNow) == "admin");

    // Never a credential — the whole separation rests on this.
    CHECK(sessions.resolveAt(token, kNow) != "MBAAAAAAAAAAAAAAAAAA");
}

TEST_CASE("an unknown token resolves to nothing", "[session]") {
    SessionStore sessions;
    sessions.openAt("admin", kNow);

    CHECK(sessions.resolveAt("", kNow).empty());
    CHECK(sessions.resolveAt("00000000", kNow).empty());
}

TEST_CASE("tokens are unguessable and distinct", "[session]") {
    SessionStore          sessions;
    std::set<std::string> tokens;
    for (int i = 0; i < 100; ++i) tokens.insert(sessions.openAt("admin", kNow));
...
TEST_CASE("a session expires once its lifetime is up", "[session]") {
    ...
    const std::string token = sessions.openAt("admin", kNow);
    CHECK(sessions.resolveAt(token, kNow + 59) == "admin");

    // Refused at the boundary and after it, without anything having swept.
    CHECK(sessions.resolveAt(token, kNow + 60).empty());
    CHECK(sessions.resolveAt(token, kNow + 10'000).empty());
}

TEST_CASE("expired sessions are collected when a new one opens", "[session]") {
    SessionStore sessions(60);

    for (int i = 0; i < 5; ++i) sessions.openAt("admin", kNow);
...
TEST_CASE("signing out invalidates the token immediately", "[session]") {
    ...
    const std::string token = sessions.openAt("admin", kNow);
    REQUIRE(sessions.resolveAt(token, kNow) == "admin");

    sessions.close(token);
    CHECK(sessions.resolveAt(token, kNow).empty());
...
TEST_CASE("signing out one session leaves the others alone", "[session]") {
    ...
    const std::string first  = sessions.openAt("admin", kNow);
    const std::string second = sessions.openAt("admin", kNow);

    sessions.close(first);
    CHECK(sessions.resolveAt(first, kNow).empty());
    CHECK(sessions.resolveAt(second, kNow) == "admin");
}
